...
#   
#   2. 스크립트 실행 (fu 파라미터 필수):
#      ./hr_analysis_manager.sh 9   # fu=9
#      ./hr_analysis_manager.sh 10  # fu=10
#   
#   3. 백그라운드 실행 (프로그래스 바 확인 가능):
#      # tmux 세션에서 실행 (권장)
#      tmux new-session -d -s hr_analysis './hr_analysis_manager.sh 9'
#      
#      # 프로그래스 바 확인 (다른 터미널에서)
#      tmux attach-session -t hr_analysis
#      
#      # tmux 세션에서 나가기 (프로세스는 계속 실행): Ctrl+B, D
#      # 세션 종료: tmux kill-session -t hr_analysis
#   
#   4. nohup 백그라운드 실행 (프로그래스 바 확인 불가):
#      nohup ./hr_analysis_manager.sh 9 > /dev/null 2>&1 &
...
# fu 파라미터 설정 (필수 파라미터)
if [ -z "$1" ]; then
    echo "ERROR: fu 파라미터가 필요합니다." >&2
    echo "사용법: $0 <fu>" >&2
    echo "예시: $0 9   # fu=9" >&2
    echo "예시: $0 10  # fu=10" >&2
    exit 1
fi
FU=$1
...
# 로그 파일 설정 (fu 기반 동적 생성)
LOG_DIR="/home/hashjamm/results/disease_network"
SCRIPT_BASENAME=$(basename "$0" .sh)
LOG_FILE="$LOG_DIR/${SCRIPT_BASENAME}_fu${FU}_history.log"
export LOG_FILE  # 서브셸에서 사용할 수 있도록 export
export LOG_DIR   # 서브셸에서 사용할 수 있도록 export
export SCRIPT_BASENAME  # 서브셸에서 사용할 수 있도록 export

# 진행 상황 추적 관련 설정 (fu 기반 동적 생성)
DB_COMPLETED_FILE="/home/hashjamm/results/disease_network/hr_job_queue_db_fu${FU}/completed_jobs.duckdb"
DB_COMPLETED_FOLDER="/home/hashjamm/results/disease_network/hr_job_queue_db_fu${FU}/completed_jobs/"
...
get_system_resources() {
    ...
    # R 프로세스 메모리 사용량 (hr_calculator_engine.R 메인 프로세스만)
    local r_mem_kb=$(ps aux | grep -E "hr_calculator_engine.R" | grep -v "worker.rank" | grep -v grep | awk '{sum+=$6} END {print sum}')
    local r_mem_gb="0.0"
...
cleanup() {
    ...
    # 방법 2: 프로세스 패턴으로 직접 찾기 (실제 R 프로세스: R --file=hr_calculator_engine.R)
    R_PID=$(pgrep -f "hr_calculator_engine.R" | grep -v "worker.rank" | head -1)
    ...
    if [ ! -z "$RSCRIPT_PID" ]; then
        ...
        SCOPE_NAME=$(systemctl list-units --type=scope --state=running 2>/dev/null | grep -E "hr_calculator|run-r[0-9a-f]" | awk '{print $1}' | head -1)
    ...
    pkill -P $$ -f "hr_calculator_engine.R" 2>/dev/null
...
# 시작 메시지
log "=========================================================="
log "HR Calculator 자동 재시작 래퍼 시작"
log "=========================================================="
log "Follow-up: $FU"
...
while [ $RESTART_COUNT -lt $MAX_RESTARTS ]; do
    ...
    log "R 스크립트 실행 중... (메모리 제한: 100G, fu=$FU)"
    ...
    ACTUAL_R_PID=$(pgrep -f "hr_calculator_engine.R" | grep -v "worker.rank" | head -1)
    ...
    (
        ...
        # fu 파라미터 확인 (필수)
        if [ -z "$FU" ]; then
            echo "ERROR: fu 파라미터가 설정되지 않았습니다." >&2
            exit 1
        fi
        export FU  # 서브셸에서도 사용 가능하도록 export
        ...
        if [ -z "$LOG_FILE" ]; then
            LOG_DIR="/home/hashjamm/results/disease_network"
            SCRIPT_BASENAME="hr_analysis_manager"
            LOG_FILE="$LOG_DIR/${SCRIPT_BASENAME}_fu${FU}_history.log"
        fi
        ...
        if [ ! -z "$TOTAL_JOBS" ] && [ "$TOTAL_JOBS" -gt 0 ]; then
            ...
            # 프로세스 패턴: hr_calculator_engine_v3.R 또는 hr_calculator_engine.R 모두 매칭
            while pgrep -f "hr_calculator_engine" | grep -v "worker.rank" | grep -v grep > /dev/null 2>&1; do

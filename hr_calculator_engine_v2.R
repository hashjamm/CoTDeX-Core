# ============================================================================
# [v11.2 수정 사항] 데이터 소실 방지를 위한 aggregation 방식 복원
# ============================================================================
# 
# v11.1 문제점:
...
#
# v11.2 수정 사항:
# [5번 - 필수] Edge 매핑 취합 방식을 pre 버전으로 복원
#   - 각 청크 파일을 하나씩 읽어서 검증 후 취합
#   - 실패한 파일은 건너뛰고 성공한 파일만 취합
#   - 취합 완료 후 청크 파일 삭제 (데이터 소실 방지)
#   - 메모리 문제는 있더라도 데이터 무결성 보장
#
# v11.2 이후 이슈 (aggregate_results_mappings):
...
process_batch <- function(
            ...
            tryCatch({
                ...
                if(nrow(clean_data) > 0) {
                    ...
                    tryCatch({
                        ...
                        # NULL 반환 케이스 처리
                        # [v11.1 수정] <<- 대신 parent.frame() 사용 (워커 환경에서 안정적)
                    ...
                    # [v11.1 수정] if-else 블록은 같은 스코프이므로 <- 사용 가능
    ...
    tryCatch({
        # [v11.1 수정] 배치 쓰기 순서 변경: HR 결과 먼저, 완료 로그 마지막
        ...
        # 4. 완료 로그 쓰기 (마지막 - HR 결과나 통계 실패 로그가 실제로 기록된 경우에만)
        ...
        if (!is.null(con_comp)) {
            ...
            # HR 결과나 통계 실패 로그가 있는 작업만 완료 로그에 기록
            valid_completed_keys <- intersect(completed_keys, c(hr_keys, stat_fail_keys))
...
#
# 2. DuckDB COPY 방식 시도 및 실패 (v11.1 시도)
...
#
# 3. 현재 방식의 한계 (v11.2)
#    - 현재: pre 버전 방식으로 복원 (각 청크를 하나씩 읽어서 검증 후 취합)
#    - 장점:
...
#
# [결정 사항]
# - Aggregate 로직을 별도 스크립트로 분리하기로 결정 (v11.3 구현 완료)
...
#
# [현재 상태]
# - v11.3: Edge 매핑 데이터 취합 로직을 aggregate_edge_mappings.R로 분리 완료
...
cat("\n--- [단계 2] 최종 '결과물' 취합 시작 ---\n")
        ...
        # --- 3. Edge 매핑 데이터 취합 ---
        # [v11.3 변경] Edge 매핑 데이터 취합 로직을 별도 스크립트로 분리
        ...
        # - 안전성: 삭제 실패 시 경고 메시지 출력 및 실패한 파일 목록 표시
        # - 참고: 최종 parquet 파일은 삭제하지 않으므로 이어쓰기 기능에 영향 없음
        cat("\n4. HR 임시 청크 파일 삭제 중...\n")
        if (length(hr_chunk_files) > 0) {
            # file.remove()의 반환값 확인 (TRUE = 성공, FALSE = 실패)
            removal_results <- file.remove(hr_chunk_files)
            successful_removals <- sum(removal_results)
            failed_removals <- sum(!removal_results)
            
            if (failed_removals > 0) {
                failed_files <- hr_chunk_files[!removal_results]
                cat(sprintf("   ⚠️  경고: %d개 파일 삭제 실패\n", failed_removals))
                cat(sprintf("      실패한 파일 (처음 10개): %s\n", 
                           paste(basename(failed_files[1:min(10, length(failed_files))]), collapse = ", ")))
                if (length(failed_files) > 10) {
                    cat(sprintf("      ... (총 %d개 중 10개만 표시)\n", length(failed_files)))
                }
            }
            
            if (successful_removals > 0) {
                cat(sprintf("   ✓ %d개 HR 청크 파일 삭제 완료\n", successful_removals))
            }
        } else {
            cat("   - 삭제할 HR 청크 파일 없음\n")
        }
        # Map 청크 파일은 aggregate_edge_mappings.R에서 처리 후 삭제
...
paths <- list(
        ...
        results_hr_folder = "/home/hashjamm/results/disease_network/hr_results_fu10/",
        results_mapping_folder = "/home/hashjamm/results/disease_network/hr_mapping_results_fu10/",
        db_completed_folder = "/home/hashjamm/results/disease_network/hr_job_queue_db_fu10/completed_jobs/",
        db_completed_file = "/home/hashjamm/results/disease_network/hr_job_queue_db_fu10/completed_jobs.duckdb",
        db_system_failed_folder = "/home/hashjamm/results/disease_network/hr_job_queue_db_fu10/system_failed_jobs/",
        db_system_failed_file = "/home/hashjamm/results/disease_network/hr_job_queue_db_fu10/system_failed_jobs.duckdb",
        db_stat_failed_folder = "/home/hashjamm/results/disease_network/hr_job_queue_db_fu10/stat_failed_jobs/",
        db_stat_failed_file = "/home/hashjamm/results/disease_network/hr_job_queue_db_fu10/stat_failed_jobs.duckdb"
    )

...
#
# engine_v4 수정 사항:
# [11번 - 필수] Edge 매핑 데이터 수집 조건 수정 (RR 프로세스와 일치)
#   - 문제: engine_v3까지 edge_pids와 edge_index_key_seq가 case==1만 조건으로 수집
#     → 원인 질병을 가진 모든 사람 포함 (outcome 발생 여부 무관)
#     → 엣지가 아닌 노드 데이터에 가까움
#   - RR 프로세스 비교:
#     * RR의 edge_pids: case==1 & outcome_status==1 (양쪽 질병 모두 발생한 경우만)
#     * HR의 edge_pids (v3): case==1 (원인 질병만 가진 경우도 포함) ❌
#     * HR의 edge_key_seq (v3): case==1 & status==1 ✅ (이미 올바름)
#   - 수정 내용:
#     * edge_pids: case==1 → case==1 & status==1
#       → 원인 질병을 가진 사람 중 결과 질병도 발생한 사람만 수집
#     * edge_index_key_seq: case==1 → case==1 & status==1
#       → 엣지에 해당하는 사람의 index_key_seq만 수집
#     * edge_key_seq: 변경 없음 (이미 case==1 & status==1 조건)
#   - 의미: 엣지 정의를 RR과 일치시켜 "cause와 outcome이 모두 발생한 경우"만 수집
#   - 적용 위치: process_batch 함수 내 매핑 데이터 추출 부분 (Line 314-315)
#
# ============================================================================
        ...
        for (i in 1:nrow(batch_jobs)) {
            ...
            tryCatch({
                ...
                if (!is.null(hr_result)) {
                    ...
                    # Mapping 데이터
                    # [engine_v4 수정] edge_pids와 edge_index_key_seq도 RR과 일치하도록 조건 수정
                    # - 엣지 정의: cause와 outcome이 모두 발생한 경우만 (case==1 & status==1)
                    key <- paste(cause_abb, outcome_abb, fu, sep = "_")
                    pids <- clean_data[case == 1 & status == 1, .(person_id)]  # [v4 수정] outcome 발생한 경우만
                    idx_key <- clean_data[case == 1 & status == 1, .(index_key_seq)]  # [v4 수정] outcome 발생한 경우만
                    out_key <- clean_data[case == 1 & status == 1, .(key_seq)]  # 이미 올바름 (변경 없음)
...
ensure_required_directories <- function(paths) {
    # 결과 폴더들
    dir.create(paths$results_hr_folder, recursive = TRUE, showWarnings = FALSE)
    ...
    dir.create(paths$db_stat_failed_folder, recursive = TRUE, showWarnings = FALSE)
    dir.create(paths$db_stat_excluded_folder, recursive = TRUE, showWarnings = FALSE)
    
    # 중앙 DB 파일의 부모 디렉토리들
    dir.create(dirname(paths$db_completed_file), recursive = TRUE, showWarnings = FALSE)
    dir.create(dirname(paths$db_system_failed_file), recursive = TRUE, showWarnings = FALSE)
    dir.create(dirname(paths$db_stat_failed_file), recursive = TRUE, showWarnings = FALSE)
    dir.create(dirname(paths$db_stat_excluded_file), recursive = TRUE, showWarnings = FALSE)
...
result <- main(
    paths = paths, fu = fu, 
    n_cores = 70, # CPU 및 디스크 IO 부하 (안정성 우선: 60 → 40)
    batch_size = 10000, # 메모리 부하 (안정성 우선: 100000 → 50000)
    chunks_per_core = 50, # 코어당 청크 수 (안정성 우선: 20 → 10, 기본값: 3, Spark/Dask 표준)
    warming_up = FALSE
)

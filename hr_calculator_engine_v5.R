...
# 
# 배경:
# - hr_rr_mapping_validation_engine_v3.R을 통해 hr 계산 과정에서 제거되는 
#   pid(person_id)들의 매핑 데이터를 확보할 수 있게 되었습니다.
# - 이를 통해 간접적으로 hr 쪽 매핑 데이터를 확보할 수 있게 됨에 따라,
#   본 엔진은 통계 결과 데이터 수집만을 목적으로 생성되었습니다.
# 
# 주요 목적:
# - HR(Hazard Ratio) 통계 분석 결과 데이터 수집
# - 각 질병 쌍(cause-outcome pair)에 대한 HR 통계량 계산 및 저장
# - 매핑 데이터는 hr_rr_mapping_validation_engine_v3.R을 통해 별도로 확보
#
# ============================================================================
...
process_batch <- function(
    ...
    progressr::with_progress({
        ...
        for (i in 1:nrow(batch_jobs)) {
            ...
            tryCatch({
                ...
                # --- [engine_v3 신규] stat_excluded 체크: case=1 & status=1이 없는 경우 HR 분석 스킵 ---
    ...
    # 메모리 정리 (메모리 파편화 방지)
    rm(batch_hr_results, batch_completed_jobs, batch_system_failed_jobs, batch_stat_failed_jobs, batch_stat_excluded_jobs) # [engine_v3 신규]
    gc()

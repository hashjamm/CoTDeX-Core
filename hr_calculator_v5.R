...
#
# 2. '적극적 예열' (Active Warm-up) -> 수동적 예열로 변경 (OS 디스크 캐시 예열)
#    - 적극적 예열 방식으로 outcome table을 메모리에 로드하는 대신, OS 디스크 캐시에 예열합니다.
#    - 하이브리드 방식에서 오히려 멀티세션으로의 outcome table 전달이 잘 이루어지지 않아 멀티세션 자체가 작동을 안하는 것을 확인.
#    - 1회 해당 파일을 읽어 OS 디스크 캐시에 예열하는 방식으로 변경.
#
...
process_one_pair <- function(
    ...
    arrow::write_parquet(hr_result, file.path(results_hr_folder_path, filename_hr))
...
main <- function(paths = paths, fu, n_cores = 90) {
    ...
    # ====================================================================
    # 스크립트 시작 시 임시 폴더를 생성하는 코드 추가
    # ====================================================================
    if (!dir.exists(paths$temp_slices_folder)) {
        dir.create(paths$temp_slices_folder, recursive = TRUE)
        cat(sprintf("--- 임시 폴더 생성 완료: %s ---\n", paths$temp_slices_folder))
    }
    # ====================================================================
...
# inspect_one_pair_data <- function(cause_abb, outcome_abb, fu, paths) {
    
#     cat(sprintf("--- [검사 시작] Cause: %s, Outcome: %s ---\n", cause_abb, outcome_abb))
    
#     # --- 1. 데이터 로드 ---
#     con <- duckdb::dbConnect(duckdb::duckdb())
#     on.exit(duckdb::dbDisconnect(con, shutdown = TRUE))

#     matched_parquet_file_path <- file.path(paths$matched_parquet_folder, sprintf("matched_%s.parquet", tolower(cause_abb)))
#     outcome_parquet_file_path <- paths$outcome_parquet_file
...
#     # --- 3. 최종 결과 요약 출력 ---
#     cat("\n--- 최종 Status 분포 ---\n")
#     print(table(status = clean_data$status, useNA = "ifany"))
...
# disease_codes <- get_disease_codes_from_path(file.path(paths$matched_parquet_folder))
# which(disease_codes == "E11")
# which(disease_codes == "G59")
# inspect_one_pair_data("E11", "G59", 10, paths)

perform_hr_analysis(inspect_one_pair_data("E11", "G59", 10, paths), 10, "E11", "G59")

# ============================================================================
# HR Calculator - 최종 아키텍처 v7.0 (DuckDB I/O + Job Queue 전략 + 워커별 로그 Queue 분리(v6에서는 성공의 경우는 중앙, 실패는 분리))
# ============================================================================
#
...
process_one_pair <- function(
    ...
    db_completed_folder_path # 중앙 성공 로그 경로
    ) {
    ...
    # --- 5. Job Queue 상태 '중앙' 업데이트 X -> 워커별 업데이트 ---
    db_completed_chunk_path <- file.path(db_completed_folder_path, sprintf("completed_chunk_%s.duckdb", worker_pid))
    con_completed <- dbConnect(duckdb::duckdb(), dbdir = db_completed_chunk_path, read_only = FALSE)
    dbWriteTable(
        con_completed, 
        "jobs", 
        data.frame(cause_abb = cause_abb, outcome_abb = outcome_abb, fu = fu), 

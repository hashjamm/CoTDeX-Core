...
process_batch <- function(
    ...
    con_fail <- if (length(batch_failed_jobs) > 0) dbConnect(duckdb::duckdb(), dbdir = db_failed_chunk_path, read_only = FALSE) else NULL
    ...
    on.exit({
        ...
        if (!is.null(con_fail)) dbDisconnect(con_fail, shutdown = TRUE)
    })
    ...
    if (!is.null(con_fail)) dbExecute(con_fail, "BEGIN TRANSACTION;")

    # 데이터 쓰기 (tryCatch로 묶어 하나라도 실패하면 롤백)
    tryCatch({
        if (!is.null(con_hr)) {
            dbWriteTable(con_hr, "hr_results", bind_rows(batch_hr_results), append = TRUE)
        }
        if (!is.null(con_map)) {
            # 테이블 생성 (최초 실행 시 필요할 수 있음)
        ...
        if (!is.null(con_fail)) dbExecute(con_fail, "COMMIT;")
        
    }, error = function(e) {
        # 쓰기 중 오류 발생 시 롤백 (데이터 일관성 유지)
        ...
        if (!is.null(con_fail)) try(dbExecute(con_fail, "ROLLBACK;"), silent = TRUE)
        # 에러를 다시 던져서 상위 tryCatch가 잡도록 함 (선택 사항)
        stop(e) 
    })

    return(TRUE) # 배치 처리 성공 (또는 롤백 후에도 성공으로 간주)
}
...
main <- function(
    ...
    n_cores = 15, 
    batch_size = 500, 
    max_batches_per_run = 200, # [v10.0 신규] 한 사이클당 실행할 배치 수
    rest_time_sec = 60,        # [v10.0 신규] 사이클 사이 휴식 시간 (초)
    warming_up = FALSE         # [v10.0 신규] OS 캐시 예열 여부
) {
    ...
    # [v10.0 신규] while 루프 시작 (모든 작업이 끝날 때까지 반복)
    while(TRUE) {
        cat("\n==========================================================\n")
        cat(sprintf("--- %s: 새 작업 사이클 시작 ---\n", Sys.time()))
        
        # --- 단계 1: 사전 로그 취합 (이어하기 준비) ---
        # (paths 리스트에 정의된 경로 사용)
        pre_aggregate_logs( 
        ...
        remaining_jobs <- run_hr_analysis(
            ...
            db_failed_folder_path = paths$db_failed_folder     # '쓰기'용
        )
        ...
        # --- [v10.0 신규] 작업 완료 여부 확인 ---
        if (remaining_jobs == 0) {
            cat("\n--- 모든 배치가 완료되었습니다! ---\n")
            break # while 루프 종료
        }
        
        # --- [v10.0 신규] 자동 휴식 (OS 메모리 정리 시간) ---
        cat(sprintf("\n--- %d개 작업 남음. %d초간 휴식 후 다음 사이클 시작... ---\n", remaining_jobs, rest_time_sec))
        Sys.sleep(rest_time_sec)
    ...
    cat(sprintf("실패한 작업 로그 (중앙): %s\n", paths$db_failed_file))
}

main(
    paths = paths, fu = 10, 
    n_cores = 18, # CPU 및 디스크 IO 부하
    batch_size = 150, # 메모리 부하,
    max_batches_per_run = 100, # [v10.0 신규] 한 사이클당 실행할 배치 수
    rest_time_sec = 30,        # [v10.0 신규] 사이클 사이 휴식 시간 (초)
    warming_up = FALSE         # [v10.0 신규] OS 캐시 예열 여부
    )
...
#' 중앙 실패 로그 파일 확인
#'
#' @param log_path 'failed_jobs.duckdb' 파일의 전체 경로
#' @param n 반환할 행의 수 (기본 10)
#' @param tail TRUE면 마지막 n개 (최신순), FALSE면 처음 n개 (오래된순)
#'
check_failed_log <- function(log_path, n = 10, tail = TRUE) {
    if (!file.exists(log_path)) {
        cat(sprintf("실패 로그 파일이 아직 생성되지 않았습니다: %s\n", log_path))
        return(invisible(NULL))
    }
...
# check_failed_log(paths$db_failed_file)
# check_failed_log(paths$db_failed_file, n = 5, tail = FALSE)
...
# check_success_chunk(paths$db_completed_folder, pid = 97336, n = 3)

check_failed_chunk(paths$db_failed_folder, pid = 98201)
check_success_log(paths$db_completed_file, n = 5, tail = FALSE)
...
# check_failed_log(paths$db_failed_file, n = 1000, tail = FALSE)$error_msg %>% unique()

# 실패 로그의 고유 오류 메시지 추출
{con <- dbConnect(duckdb::duckdb(), dbdir = paths$db_failed_file, read_only = TRUE); 
 on.exit(dbDisconnect(con, shutdown = TRUE)); 

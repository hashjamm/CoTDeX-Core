...
run_hr_analysis <- function(
    ...
    log_aggregation_interval, # [v10.0 신규] 로그 취합 주기 (몇 개 배치 처리 후 로그 취합할지)
    matched_parquet_folder_path, 
    ...
    num_batches_this_run <- min(num_batches, log_aggregation_interval)
    batches_indices_this_run <- batches_indices[1:num_batches_this_run]

    cat(sprintf("--- 이번 실행에서 %d개 배치 중 %d개 처리 시작 (각 배치는 작은 청크로 분할하여 %d개 코어가 동적으로 처리) ---\n", 
        num_batches, num_batches_this_run, n_cores))
    ...
    progressr::with_progress({
        ...
        for (batch_idx in 1:length(batches_indices_this_run)) {
            ...
            # 작은 청크 크기 계산: 코어 수의 2-3배만큼 청크 생성하여 부하 균형 향상
            chunks_per_core <- 3  # 코어당 청크 수 (Spark, Dask 등에서도 2-3 사용)
...
main <- function(
    ...
    n_cores = 15, 
    batch_size = 500, 
    log_aggregation_interval = 200,  # [v10.3] 로그 취합 주기 (몇 개 배치 처리 후 로그 취합할지)
    warming_up = FALSE
) {
    ...
    remaining_jobs <- run_hr_analysis(
        ...
        log_aggregation_interval = log_aggregation_interval, # [v9.4]
    ...
    if (remaining_jobs == 0) {
        ...
        # log_aggregation_interval개 처리 후 완전히 종료하여 메모리 파편화 해소
        # 사용자는 스크립트를 다시 실행하면 자동으로 이어서 진행됨
        cat("\n==========================================================\n")
        cat("--- 메모리 파편화 방지를 위한 프로세스 종료 ---\n")
        cat("==========================================================\n")
        cat(sprintf("이번 사이클에서 %d개 배치 (약 %d개 작업) 처리 완료\n", 
                   log_aggregation_interval, log_aggregation_interval * batch_size))
...
result <- main(
    ...
    log_aggregation_interval = 100,  # [v10.3] 로그 취합 주기 (몇 개 배치 처리 후 로그 취합할지)
    warming_up = FALSE
)

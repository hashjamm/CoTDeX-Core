#!/bin/bash
# ============================================================================
# v10 매핑 데이터 복구 스크립트 실행 래퍼 (자동 재시작 기능 포함)
# ============================================================================
# 목적: v10_recover_20251110.R 스크립트를 실행하여
#       1. v5 청크에서 매핑 데이터 복구
#       2. 누락 조합에 대한 edge mapping 생성
# 
# [실행 방법]
...
#   
#   2. 스크립트 실행:
#      ./v10_recover_20251110.sh
#   
#   3. 백그라운드 실행 (tmux 권장):
#      tmux new-session -d -s v10_recover './v10_recover_20251110.sh'
#      tmux attach-session -t v10_recover
# ============================================================================

# 설정
CONDA_ENV="disease_network_data"  # Conda 가상환경 이름
R_SCRIPT="/home/hashjamm/codes/disease_network/hr_fu10_recover.R"
MAX_RESTARTS=100
RESTART_COUNT=0
...
# 전역 변수: 프로그레스바 추적 프로세스 PID (cleanup에서 사용)
PROGRESS_TRACK_PID=""
...
get_system_resources() {
    ...
    # R 프로세스 메모리 사용량 (v10_recover_20251110.R 메인 프로세스만)
    local r_mem_kb=$(ps aux | grep -E "v10_recover_20251110.R" | grep -v "worker.rank" | grep -v grep | awk '{sum+=$6} END {print sum}')
    local r_mem_gb="0.0"
...
show_detailed_progress() {
    ...
    # 처음 호출 시 두 줄의 공간을 확보하고 커서를 첫 줄로 올리기
    if [ "$PROGRESS_BAR_INITIALIZED" -eq 0 ]; then
        # 두 줄 확보
        printf "\n\n"
        # 두 줄 위로 올라가서 첫 줄로 복귀 (\033[1A를 두 번 사용)
    ...
    # total_jobs는 missing_combinations 파일에서 읽은 전체 누락 조합 수
    ...
    if [ ! -z "$total_batches_from_log" ] && [ "$total_batches_from_log" -gt 1 ]; then
        # 첫 번째 배치 정보 찾기
        local first_batch_line=$(grep "\[배치 1/" "$log_file" 2>/dev/null | grep "작은 청크 기반 할당" | head -1)
    ...
    # 전체 배치 수 계산 (로그에서 파싱한 값을 우선 사용, 없으면 batch_size로 계산)
    local total_batches=0
    if [ ! -z "$total_batches_from_log" ] && [ "$total_batches_from_log" -gt 0 ]; then
        total_batches=$total_batches_from_log
    elif [ "$total_jobs" -gt 0 ] && [ "$batch_size" -gt 0 ]; then
        # 파싱 실패 시 batch_size로 계산
        total_batches=$((total_jobs / batch_size))
        if [ $((total_jobs % batch_size)) -gt 0 ]; then
            total_batches=$((total_batches + 1))
        fi
    fi
    
    # 남은 작업 중 완료된 배치 수 계산 (batch_size 사용)
    local completed_batches=0
    if [ "$batch_size" -gt 0 ] && [ "$new_completed_jobs" -gt 0 ]; then
        completed_batches=$((new_completed_jobs / batch_size))
    fi
    ...
    if [ ! -z "$total_batches" ] && [ "$total_batches" -gt 0 ] && [ "$batch_size" -gt 0 ]; then
        ...
        # 현재 배치 내부 진행률 계산
        # 현재 배치의 시작 작업 수 = 완료된 배치 수 * batch_size (배치당 최대 작업 수)
        local current_batch_start_jobs=$((completed_batches * batch_size))
        
        # 현재 배치 내 완료 작업 수 = 새로 완료된 작업 수 - 현재 배치 시작 작업 수
        local batch_internal_completed=0
        if [ "$new_completed_jobs" -gt "$current_batch_start_jobs" ]; then
            batch_internal_completed=$((new_completed_jobs - current_batch_start_jobs))
        fi
        
        # 현재 배치 내 진행률 계산
        local batch_internal_pct=0
        if [ "$batch_size" -gt 0 ]; then
            batch_internal_pct=$((batch_internal_completed * 100 / batch_size))
        ...
        if [ ! -z "$eta" ] && [ "$eta" != "N/A" ]; then
            first_line="${first_line} | ETA: $eta"
        fi
        
        # 두 번째 줄: 배치 상세 정보 및 시스템 리소스
        local second_line="[배치 진행] [$batch_bar] 배치 $((current_batch + 1))/$total_batches ($batch_pct%)"
        if [ "$current_batch" -lt "$total_batches" ]; then
            second_line="${second_line} | 배치 내부: [$batch_internal_bar] ($batch_internal_pct%)"
        fi
        second_line="${second_line} | $system_info"
        
        # 두 줄 모두 지우고 새로 출력
        printf "\r\033[K%s\n\r\033[K%s" "$first_line" "$second_line"
    else
        # 배치 정보가 없으면 간단한 프로그래스 바만 표시
        local progress_pct=$overall_pct
        if [ "$progress_pct" -gt 100 ]; then progress_pct=100; fi
        ...
        local line="[작업 진행] [$bar] $new_completed_jobs/$total_jobs 완료 ($progress_pct%)"
        if [ ! -z "$eta" ] && [ "$eta" != "N/A" ]; then
            line="${line} | ETA: $eta"
        fi
        line="${line} | $system_info"
...
# 함수: missing_combinations 파일에서 전체 작업 수 읽기
parse_total_jobs_from_file() {
    local missing_file="$OUTPUT_FOLDER/missing_combinations_${FU}.parquet"
    ...
    # Parquet 파일에서 행 수 읽기
    local count=$(echo -e ".mode csv\n.header off\nSELECT COUNT(*) FROM read_parquet('$missing_file');" | $duckdb_cmd 2>/dev/null | head -1 | sed 's/[^0-9]//g')
...
parse_batch_info_from_log() {
    ...
    # 전체 배치 수 파싱: "3. 배치 처리 시작: %d개 배치"
    local total_batches=$(grep -o "[0-9]*개 배치 (각 배치는" "$log_file" 2>/dev/null | grep -o "[0-9]*" | head -1)
...
parse_batch_size_from_r_script() {
    local r_script=$1
    # "batch_size = [숫자]" 패턴 찾기 (주석 제거 후)
    local batch_size=$(grep -o "batch_size[[:space:]]*=[[:space:]]*[0-9]*" "$r_script" 2>/dev/null | grep -o "[0-9]*" | head -1)
...
if [ -z "$CONDA_DEFAULT_ENV" ] || [ "$CONDA_DEFAULT_ENV" != "$CONDA_ENV" ]; then
    ...
    if [ ! -d "$CONDA_ENV_PATH" ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ERROR: conda 환경 '$CONDA_ENV'를 찾을 수 없습니다: $CONDA_ENV_PATH" >&2
        exit 1
    fi
...
cleanup() {
    ...
    # 프로세스 패턴으로 직접 찾기
    R_PID=$(pgrep -f "v10_recover_20251110.R" | grep -v "worker.rank" | head -1)
    ...
    # systemd-run scope 종료
    if [ ! -z "$RSCRIPT_PID" ]; then
        SCOPE_NAME=$(systemctl list-units --type=scope --state=running 2>/dev/null | grep -E "v10_recover|run-r[0-9a-f]" | awk '{print $1}' | head -1)
    ...
    # 프로그레스바 추적 프로세스 종료
    if [ ! -z "$PROGRESS_TRACK_PID" ] && kill -0 $PROGRESS_TRACK_PID 2>/dev/null; then
        log "프로그레스바 추적 프로세스 (PID: $PROGRESS_TRACK_PID) 종료 중..."
        kill -TERM $PROGRESS_TRACK_PID 2>/dev/null
        sleep 1
        if kill -0 $PROGRESS_TRACK_PID 2>/dev/null; then
            kill -9 $PROGRESS_TRACK_PID 2>/dev/null
        fi
    fi
    
    # R 프로세스 그룹 전체 종료 (최종 안전장치)
    pkill -P $$ -f "v10_recover_20251110.R" 2>/dev/null
...
# 시작 메시지
log "=========================================================="
log "v10 매핑 데이터 복구 스크립트 시작 (자동 재시작 기능 포함)"
log "=========================================================="
...
while [ $RESTART_COUNT -lt $MAX_RESTARTS ]; do
    ...
    ACTUAL_R_PID=$(pgrep -f "v10_recover_20251110.R" | grep -v "worker.rank" | head -1)
    ...
    (
        ...
        if [ -z "$DB_COMPLETED_FILE" ]; then
            DB_COMPLETED_FILE="/home/hashjamm/results/disease_network/hr_job_queue_db_v10_recovered/completed_jobs.duckdb"
        fi
        if [ -z "$DB_COMPLETED_FOLDER" ]; then
            DB_COMPLETED_FOLDER="/home/hashjamm/results/disease_network/hr_job_queue_db_v10_recovered/completed_jobs/"
        fi
        ...
        if [ -z "$LOG_FILE" ]; then
            LOG_DIR="/home/hashjamm/results/disease_network"
            SCRIPT_BASENAME="v10_recover_20251110"
            LOG_FILE="$LOG_DIR/${SCRIPT_BASENAME}_history.log"
        fi
        if [ -z "$CONDA_ENV_PATH" ] && [ ! -z "$CONDA_BASE" ] && [ ! -z "$CONDA_ENV" ]; then
            CONDA_ENV_PATH="$CONDA_BASE/envs/$CONDA_ENV"
        fi
        # START_COMPLETED_JOBS 변수도 상속 확인
        if [ -z "$START_COMPLETED_JOBS" ]; then
            START_COMPLETED_JOBS=0
        fi
        # R_SCRIPT 변수도 상속 확인
        if [ -z "$R_SCRIPT" ]; then
            R_SCRIPT="/home/hashjamm/codes/disease_network/v10_recover_20251110.R"
        fi
        
        # 전체 작업 수를 파일에서 읽기 (missing_combinations 파일)
        max_wait=30
        waited=0
        while [ $waited -lt $max_wait ]; do
            TOTAL_JOBS=$(parse_total_jobs_from_file)
        ...
        if [ ! -z "$TOTAL_JOBS" ] && [ "$TOTAL_JOBS" -gt 0 ]; then
            ...
            while pgrep -f "v10_recover_20251110.R" | grep -v "worker.rank" > /dev/null 2>&1; do
                # show_detailed_progress 함수가 자체적으로 두 줄을 지우고 출력하므로
